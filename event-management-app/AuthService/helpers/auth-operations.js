...
export const registerUserHelper = async (userData) => {
    try {
        ...
        Promise.resolve()
            .then(() =>
                sendVerificationEmail(email, name, verificationToken, newUser.Id)
            )
            .catch((err) =>
                console.error(
...
export const verifyEmailHelper = async (token, userId) => {
    try {
        ...
        if (!user) {
            if (userId) {
                const verifiedUser = await findUserById(userId);
                if (verifiedUser?.UserEmail?.EmailVerified) {
                    return {
                        success: true,
